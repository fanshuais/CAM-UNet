...
class CAMixer(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5, in_ch=3):
        super().__init__()
    ...
    def forward(self, x, condition_global=None, mask=None, train_mode=False):
        ...
        if self.is_deformable:
            ...
            if condition_global is None:
                print(v.shape,condition_wind.shape)
        ...
        if self.training:
            # return out[:,:,0:h_,0:w_], torch.mean(mask, dim=1)
            return out[:, :, 0:h_, 0:w_], 2 * self.ratio * (torch.mean(torch.mean(mask, dim=1), dim=0) - 0.5) ** 2
        return out[:,:,0:h_,0:w_]
...
if __name__ == '__main__':
    x = torch.randn((1, 3, 179, 72))
    net =CAMixerSR(scale=4, n_feats=60, ratio=0.5)# CAMixer(dim=30)
    #  net.eval()
